...
- Fetch the target URL using `antigravity-seo run render_page.py "<url>" --mode auto --json` (SPA-aware SSRF-protected renderer)
- Parse with `antigravity-seo run parse_html.py --url "<url>"` to extract SEO elements
...
- [ ] URL was fetched via `antigravity-seo run render_page.py --mode auto` (not raw curl)
