...
1. Check credentials: `antigravity-seo run backlinks_auth.py --check --json`
...
5. Format output to match antigravity-seo conventions

## Tier-Based Workflow

### Tier 0 (Always Available, No Config Needed)
- Common Crawl domain metrics: `antigravity-seo run commoncrawl_graph.py <domain> --json`
  - PageRank, PageRank rank, harmonic centrality, harmonic centrality rank, crawl/ranking presence
- If known backlinks provided, verify them: `antigravity-seo run verify_backlinks.py --target <url> --links <file> --json`
...
### Tier 1 (+ Moz API)
- All Tier 0 checks
- Moz URL metrics: `antigravity-seo run moz_api.py metrics <url> --json`
  - DA, PA, Spam Score, link counts, referring domains
- Moz referring domains: `antigravity-seo run moz_api.py domains <url> --json`
- Moz anchor text: `antigravity-seo run moz_api.py anchors <url> --json`
- Moz top pages: `antigravity-seo run moz_api.py pages <domain> --json`
...
- All Tier 1 checks
- Bing inbound links: `antigravity-seo run bing_webmaster.py links <url> --json`
- For comparison between two properties registered to the same Bing account:
  `antigravity-seo run bing_webmaster.py compare <url1> <url2> --json`
...
```bash
antigravity-seo run validate_backlink_report.py --report report_data.json --json
```
